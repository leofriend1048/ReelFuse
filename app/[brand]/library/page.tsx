...
}> = React.memo(function VideoGrid({ videos, onVideoClick, selectedVideos, onVideoSelect, isSelectionMode, onCopyLink }) {
  return (
...
const VideoCard: React.FC<{
...
}> = React.memo(function VideoCard({ video, onClick, isSelected, onSelect, isSelectionMode, onCopyLink }) {
  ...
  return (
    <Card
      className={`relative rounded-lg transition-all duration-200 cursor-pointer hover:shadow-lg group ${
    ...
    >
      ...
      <div className="relative">
        ...
        <div className={showSkeleton ? 'opacity-0' : 'opacity-100'}>
          <VideoThumbnail
...
const VideoSkeleton = () => (
  <Skeleton className="w-[200px] h-[356px] rounded-lg animate-pulse" />
);
...
export default function Chat() {
  ...
  const openai = new OpenAI({
    apiKey: "sk-proj-gcEZfHk-TGv57hOa9EhcEbtSYANOfPt_Has1vhYhw08m1ovgwre7EWbTWuPjXUXy9IzP0-R9jXT3BlbkFJA9Skg0iUW0rtWocn9nscA0Y-ruBxsk2nWfB2iKljyzHrjN_09zA6QrXUBnZ5nso59R85bAQaQ",
  ...
  const handleDialogClose = () => {
    ...
    // Remove the video parameter from the URL without navigation
  ...
  const handleUpdate = async (video: Video) => {
    try {
      const { error } = await supabase
        .from('modular_clips')
        .update({ description: video.description })
  ...
  const getVideos = async () => {
    ...
    try {
      const query = supabase
        .from('modular_clips')
        .select('video_url, description, creator, poster_url, duration', { count: 'exact' })
        .eq('brand', brand);
  ...
  }, [brand, currentPage, selectedCreator]);
            ...
            <AnimatePresence mode="wait">
              ...
              >
                <div className="flex w-2/3 h-full items-center justify-center bg-white">
                  <div className="flex h-full max-w-96 items-center justify-center">
                    <VideoPlayer
                      src={videoData[currentVideoIndex].video_url}
                      poster={videoData[currentVideoIndex].poster_url}
