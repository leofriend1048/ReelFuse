...
export default function Chat() {
  const [videoData, setVideoData] = useState<{ rendered_video: string; hook_1: string; video_id: string } | null>(null);
    ...
    const getMostRecentVideo = async () => {
      try {
        const { data, error } = await supabase
          ...
          .select('rendered_video, hook_1, video_id') // Now we're fetching the video URL directly
        ...
        if (data && data.length > 0) {
          ...
          setVideoData({ rendered_video: data[0].rendered_video, hook_1: data[0].hook_1, video_id: data[0].video_id });
        }
      } catch (error) {
  ...
  async function onSubmit(data: z.infer<typeof FormSchema>) {
    ...
    const response = await fetch('https://api.airops.com/public_api/agent_apps/af35fbef-2892-4416-a1e7-94011a6a4066/chat', {
      method: 'POST',
      headers: {
...
try {
  ...
  const elevenLabsResponse = await fetch('https://api.elevenlabs.io/v1/text-to-speech/N4cd1xylk1UBax6E7yZT?output_format=mp3_22050_32', elevenLabsOptions);
        ...
        const creatomateResponse = await fetch('https://api.creatomate.com/v1/renders', {
          ...
          body: JSON.stringify({
            ...
            "modifications": {
              ...
              "Hook-1": hookScriptCopy,
              "Problem-Background": problemVideoURL,
              "Problem-1": problemScriptCopy,
              "Agitate-Background": agitateProblemVideoURL,
              "Agitate-1": agitateProblemScriptCopy,
        ...
        if (result.status === "success") {
          ...
          try {
            const { data: supabaseData, error: supabaseError } = await supabase
              .from('rendered_videos')
              .insert([
                { hook_background: hookVideoURL, hook_1: hookScriptCopy, rendered_video: finalUrl }
              ]);
...
console.error("Error parsing response data or other processing error:", error);
              ...
              <div className="flex w-full fixed bottom-0 items-center space-x-2 max-w-md p-2 mb-8">
                <Form {...form} className="flex w-full fixed bottom-0 items-center space-x-2 max-w-md p-2 mb-8">
                  <form onSubmit={form.handleSubmit(onSubmit)} className="flex w-full space-y-6">
                    ...
                    />
                     {isSubmitting ? (
              <Button className="fixed bottom-5 w-full max-w-md p-2 mb-8 rounded shadow-x text-base" disabled>
  ...
  {videoData && (
    <>
      <video width="350px" height="auto" controls preload="auto">
    ...
    <>
        <Button asChild>
        <Link href={`/video/${videoData.video_id}`}>See more</Link>
    </Button>
    </>
  )}
  ...
  <div className="space-y-2 text-center">
    <h3 className="text-2xl font-bold tracking-tighter sm:text-3xl">Hook</h3>
      {videoData && (
    <>
 <p className="text-gray-500 dark:text-gray-400">{videoData.hook_1}
      </p>
    </>
  )}
  </div>
        ...
        <div className="rounded-lg border border-gray-200 p-4 flex items-center justify-center md:p-10 dark:border-gray-800">
          <div className="space-y-2 text-center">
            ...
            <p className="text-gray-500 dark:text-gray-400">this is the problem</p>
          </div>
        </div>
        <div className="rounded-lg border border-gray-200 p-4 flex items-center justify-center md:p-10 dark:border-gray-800">
          <div className="space-y-2 text-center">
